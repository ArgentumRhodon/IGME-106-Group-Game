...
namespace IGME106GroupGame.UI.Menus
{
    class PauseUI : UserInterface
    {
        ...
        {
            images.Add(new Image(game.Assets.Get("pausedTitle"), game.Graphics.PreferredBackBufferWidth / 2, game.Graphics.PreferredBackBufferHeight / 3, HAlign.Center, VAlign.Middle));
            ...
            buttons.Add(new Button(new Image(game.Assets.Get("continueText"), game.Graphics.PreferredBackBufferWidth / 2, 575, HAlign.Center, VAlign.Top), (State state) => { ((GameState)state).IsPaused = false; }));
        }
    }
}
