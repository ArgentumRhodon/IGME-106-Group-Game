    ...
    class GameUI : UserInterface
    {
        ...
        public void UpdateHealth()
        {
            ...
            for (int i = 1; i <= player.Health; i++)
            {
                if (i % 2 == 0)
                {
                    hearts.Add(new Image(Assets.Get("heart"), 20 + (110 * ((i - 1) / 2)), 20, Alignment.Begin, Alignment.Begin, game.Graphics));
                }
                else if (i == player.Health)
                {
                    hearts.Add(new Image(Assets.Get("halfHeart"), 20 + (110 * (i / 2)), 20, Alignment.Begin, Alignment.Begin, game.Graphics));
                }
            }
